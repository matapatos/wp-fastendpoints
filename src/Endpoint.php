...
class Endpoint implements EndpointInterface
{
     ...
     * If set it can override the default WP FastEndpoints arguments.
            ...
            foreach ($capabilities as $cap) {
                if (\is_string($cap)) {
                    if (!\current_user_can($cap)) {
                        ...
                        throw new HttpException('Not enough permissions', WP_Http::FORBIDDEN, $data);
                    }
                } elseif (\is_array($cap)) {
                    if (!$cap) {
     ...
     * Lower numbers correspond with earlier execution, and functions with the same priority
     * are executed in the order in which they were added. Default value: 10.
     * @param bool $override - Flag that determines if a post is already set if it should override it
    ...
    public function callback(WP_REST_Request $req): WP_REST_Response
    {
        ...
        if (is_wp_error($result)) {
            throw HttpException::fromWpError($result);
        }

        // Post handlers.
        return rest_ensure_response($this->runHandlers($this->postHandlers, $req, $result));
    }
     ...
     * @param WP_REST_Request $req - Current REST request.
     */
    public function permissionCallback(WP_REST_Request $req): bool
    {
        try {
            $this->runHandlers($this->permissionHandlers, $req);
            return true;
        } catch (Exception $e) {
            return false;
        }
     ...
     * @throws HttpException if the user function retrieves a WP_Error.
     */
    protected function runHandlers(array &$allHandlers, ...$args)
    {
        ...
        foreach ($allHandlers as $priority => $handlers) {
            foreach ($handlers as $h) {
                ...
                if (\is_wp_error($result)) {
                    return HttpException::fromWpError($result);
                }
            }
        }
        return $result;
